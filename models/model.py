...
from utils.utils_ssim import calculate_ssim2
...
class Model:
    def __init__(self, opt: Dict[str, Any]):
        ...
        self.batchsize =self.opt['data']['train']['batch_size']
    ...
    def loss_compound(self, pre, ori, sigma=0.85):
        ...
        l1 = L1(pre, ori)
        lssim = 0.0
        lssim = torch.tensor(lssim, requires_grad=True)

        for i, j in zip(pre, ori):
            ssim = calculate_ssim2(i, j)
            lssim = lssim + (-torch.log(ssim))

        loss = sigma * l1 + (1 - sigma) * (lssim/self.batchsize)
        return loss
